/**
Copyright 2014-2025 Robert McNeel and Associates

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.

----------------------------------------------------------------------
NOTE: Do NOT modify this file directly, it is automatically generated.

Code generated at: 2025-11-21 08:02:35 UTC
----------------------------------------------------------------------

**/

using ccl;
using ccl.Attributes;
using ccl.ShaderNodes;
using ccl.ShaderNodes.Sockets;
using ccl.NodeSockets;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Security.Cryptography;
namespace ccl
{
    using cclext;
    public class Device
    {
        public IntPtr Ptr { get; private set; } = IntPtr.Zero;

        public Device() {}

        public Device(IntPtr intPtr) { Ptr = intPtr; }
        public DeviceInfo Info { get; private set; } = null;
        private Device(DeviceInfo dev, uint index) {
            Info = dev;
            Index = index;

        }
        static public uint Count => CSycles.number_devices();

        static public IEnumerable<Device> Devices
        {
            get
            {
                for (var i = 0; i < Count; i++)
                {
                    yield return GetDevice(i);
                }

#if NOTYET
                for (var j = 0; j < MultiCount; j++)
                {
                    yield return GetDevice(j + MultiOffset);
                }
#endif
            }
        }
        /// <summary>
        /// True if this is a multi device
        /// </summary>
        public bool IsMulti => Type == DeviceType.DEVICE_MULTI;

        /// <summary>
        /// True if this device is a Cuda device
        /// </summary>
        public bool IsCuda => Type == DeviceType.DEVICE_CUDA;

        /// <summary>
        /// True if this device is a Cpu
        /// </summary>
        public bool IsCpu => Type == DeviceType.DEVICE_CPU;

        /// <summary>
        /// True if this device is an OptiX device
        /// </summary>
        public bool IsOptix => Type == DeviceType.DEVICE_OPTIX;

        /// <summary>
        /// True if this device is a Metal device
        /// </summary>
        public bool IsMetal => Type == DeviceType.DEVICE_METAL;

        /// <summary>
        /// True if this device is a HIP device
        /// </summary>
        public bool IsHip => Type == DeviceType.DEVICE_HIP;

        /// <summary>
        /// True if this device is a HIPRT device
        /// </summary>
        public bool IsHipRt => Type == DeviceType.DEVICE_HIPRT;

        /// <summary>
        /// True if this device is a OneAPI device
        /// </summary>
        public bool IsOneApi => Type == DeviceType.DEVICE_ONEAPI;

        /// <summary>
        /// True if this device is a GPU
        /// </summary>
        public bool IsGpu => !IsCpu;

        public uint Index { get; private set; }
        public string Id { get { return Info.Id; }}
        public string Description { get { return Info.Description; }}
        public string Name { get { return Info.Id; }}
        public uint Num { get { return (uint)Info.Num; }}
        public DeviceType Type { get { return Info.Type; }}
        public bool DisplayDevice { get { return Info.DisplayDevice; }}

        static public bool CudaAvailable()
        {
            return Devices.Any(dev => dev.IsCuda);
        }

        static public bool OptixAvailable()
        {
            return Devices.Any(dev => dev.IsOptix);
        }

        static public bool HipAvailable()
        {
            return Devices.Any(dev => dev.IsHip || dev.IsHipRt);
        }

        static public bool OneApiAvailable()
        {
            return Devices.Any(dev => dev.IsOneApi);
        }

        static public bool MetalAvailable()
        {
            return Devices.Any(dev => dev.IsMetal);
        }

        /// <summary>
        /// Give a nice name for UI usage. PCI bus and other redundancy is removed.
        ///
        /// For a multidevice the type of multi device and its subdevices is given.
        /// </summary>
        public string NiceName
        {
            get
            {
                if (IsMulti)
                {
                    var n = string.Join(", ", (from sd in Subdevices select sd.NiceName).ToList());
                    var multiType = FirstFromMulti.Type;
                    if(multiType == DeviceType.DEVICE_OPTIX) {
                        n = n.Replace(" (Optix)", "");
                    }
                    return $"Multi ({multiType}): {n}";
                }
                if (IsCuda || IsHip || IsMetal)
                {
                    return Name.Split('_')[1];
                }
                if (IsOptix)
                    return $"{Name.Split('_')[1]} (Optix)";
                return Name;
            }
        }

        public string NiceNameSha
        {
            get
            {
                using (SHA256 sha = SHA256.Create())
                {
                    var nicenamebytes = System.Text.Encoding.UTF8.GetBytes(NiceName);
                    var hash = sha.ComputeHash(nicenamebytes);
                    var strhash = string.Concat(hash.Select(b => b.ToString("x2")));
                    return strhash;
                }
            }
        }

        static public Device GetDevice(int idx)
        {
            DeviceInfo devInfo = CSycles.get_device_info((uint)idx);
            Device d = new Device(devInfo, (uint)idx);

#if NOTYET
            if(d.Description == "Multi Device")
            {
                d.Type = DeviceType.Multi;
          }
#endif

            return d;
        }

        static public Device Default => FirstCpu;

        /// <summary>
        /// Returns the first cuda device if it exists,
        /// the default rendering device (Cpu) if not.
        /// </summary>
        static public Device FirstCuda
        {
            get
            {
                var d = (from device in Devices
                                 where device.IsCuda || device.IsMultiCuda
                                 select device).FirstOrDefault();
                return d ?? Default;
            }
        }
        /// <summary>
        /// Get the first GPU if one exists and is ready,
        /// the default rendering device (Cpu) if not.
        /// </summary>
        static public Device FirstGpu
        {
            get
            {
                var d = (from device in Devices
                                 where device.IsGpu
                                 select device).FirstOrDefault();
                return d ?? Default;

            }
        }

        public Device FirstFromMulti  {
            get
            {
                var l = Subdevices.Where((Device d) => d.Type != DeviceType.DEVICE_CPU);
                if(l.Any()) {
                    return l.First();
                }

                return FirstCpu;
            }
        }
        /// <summary>
        /// Get the first Cpu if one exists,
        /// the default rendering device (Cpu) if not.
        /// </summary>
        static public Device FirstCpu
        {
            get
            {
                var d = (from device in Devices
                                 where device.IsCpu
                                 select device).FirstOrDefault();
                return d ?? null;

            }
        }
        /// <summary>
        /// Parse given string into a list of integers.
        ///
        /// The integers should correspond to indices of devices in the
        /// global list of regular render devices.
        /// </summary>
        /// <param name="res"></param>
        /// <returns>
        /// Sorted List of indices into global list of regular render devices.
        /// </returns>
        static public List<int> IdListFromString(string res)
        {
            var l = IdSetFromString(res).ToList();
            l.Sort();
            return l;
        }

        /// <summary>
        /// Tell if given device id string constitutes a valid combo (do all device ids exist).
        /// </summary>
        /// <param name="res"></param>
        /// <returns></returns>
        static public string ValidDeviceString(string res)
        {
            if (res.Equals("-1")) return res;
            var l = IdListFromString(res);

            var existing_l = from dev in Devices select dev.Index;

            var x = from id in l where existing_l.Contains((uint)id) select id;

            return string.Join(",", x.ToList());
        }

        /// <summary>
        /// Generate string from device Index or Indices (in case of multidevice).
        ///
        /// The resulting string can be used as input to DeviceFromString().
        /// </summary>
        public string DeviceString
        {
            get
            {
                if (!IsMulti) return $"{Index}";
                return string.Join(", ", (from sd in Subdevices select sd.Index).ToList());
            }
        }
        /// <summary>
        /// Parse given string into a set of integers.
        /// </summary>
        /// <param name="res"></param>
        /// <returns>
        /// HashSet of indices into global list of regular render devices.
        /// </returns>
        static public HashSet<int> IdSetFromString(string res)
        {
            var cleaned = res.Trim().ToLowerInvariant().Split(',');
            HashSet<int> set = new HashSet<int>();
            foreach (var item in cleaned)
            {
                if (int.TryParse(item, out int x))
                {
                    if (x > -1)
                    {
                        set.Add(x);
                    }
                }
            }
            return set;
        }

        /// <summary>
        /// Convert a list of id integers to a Device list.
        ///
        /// The ids are indices into the global list of regular devices,
        /// i.e. used with GetDevice(int idx);
        /// </summary>
        /// <param name="ids"></param>
        /// <returns></returns>
        static public List<Device> DeviceListFromIntList(List<int> ids)
        {
            // TODO filter out first all ids that do not
            // match an existing device. This can happen due to hardware
            // configuration changes where an ID no longer is valid
            // after an ID was selected
            return ids.ConvertAll((int id) => GetDevice(id));
        }

        /// <summary>
        /// Get the device for given string. If the string doesn't parse correctly
        /// the first Cuda device (or Cpu if no Cuda device was found) is picked.
        /// </summary>
        /// <param name="res"></param>
        /// <returns></returns>
        static public Device DeviceFromString(string res)
        {
            var l = IdListFromString(res);

            if (l.Count == 0) return FirstGpu;
            if (l.Count == 1)
            {
                return l[0] == -1 ? FirstCuda : GetDevice(l[0]);
            }

            return CreateMultiDevice(DeviceListFromIntList(l));
        }

        /// <summary>
        /// Create a multi-device from the given Device list.
        /// </summary>
        /// <param name="idx">List of Devices</param>
        /// <returns>Device representing the multi-device</returns>
        static public Device CreateMultiDevice(List<Device> idx)
        {
            idx.Sort((Device a, Device b) =>
            {
                if (a.Index == b.Index) return 0;
                return a.Index < b.Index ? -1 : 1;
            });
            int[] sortedidx = idx.ConvertAll<int>((Device d) => ((int)d.Index)).ToArray();
            var id = -1; // NOTYET TODO CSycles.create_multidevice(sortedidx.Length, ref sortedidx);
            if (id < 0) return null;
            return GetDevice(id);
        }

        /// <summary>
        /// Test if given ID is for this device
        /// </summary>
        /// <param name="id"></param>
        /// <returns></returns>
        public bool HasId(int id)
        {
            if (Type != DeviceType.DEVICE_MULTI) return (int)Index == id;

            foreach (var sd in Subdevices)
            {
                if ((int)sd.Index == id) return true;
            }

            return false;
        }

        /// <summary>
        /// Test if given ID is for this device
        /// </summary>
        /// <param name="id"></param>
        /// <returns></returns>
        public bool HasId(uint id)
        {
            return HasId((int)id);
        }

        public override bool Equals(object obj)
        {
            var dev = obj as Device;
            if (dev == null) return false;

            return dev.Index == Index && dev.SubdeviceCount == SubdeviceCount && dev.Subdevices.SequenceEqual(Subdevices);
        }

        public override int GetHashCode()
        {
            return DeviceString.GetHashCode();
        }

        static public bool operator ==(Device a, Device b)
        {
            if (!(a is null) && b is null) return false;
            if (a is null && !(b is null)) return false;
            if (a is null && b is null) return true;
            return a.Equals(b);
        }
        static public bool operator !=(Device a, Device b)
        {
            if (!(a is null) && b is null) return true;
            if (a is null && !(b is null)) return true;
            if (a is null && b is null) return false;
            return !a.Equals(b);
        }

        /// <summary>
        /// Get the number of available Cycles render multi-devices
        /// </summary>
        static public uint MultiCount => 0; // NOTYET TODO CSycles.number_multidevices();

        /// <summary>
        /// Amount of sub-devices for this (multi-)device.
        /// </summary>
        public uint SubdeviceCount => 0; // NOTYET TODO CSycles.number_multi_subdevices((int)Id);

        /// <summary>
        /// Enumerator over subdevices of this multi devices.
        /// </summary>
        public IEnumerable<Device> Subdevices
        {
            get
            {
                for (var i = 0; i < SubdeviceCount; i++)
                {
#if NOTYET
                    var j = CSycles.get_multidevice_subdevice_id((int)Id, i);
                    yield return GetDevice((int)j);
#else
                    yield return null;
#endif
                }
            }
        }

        public bool MultiWithCpu => Subdevices.Any(dev => dev.IsCpu);
        public bool IsMultiCuda => Type == DeviceType.DEVICE_MULTI && Subdevices.Any(device => device.IsCuda);
        public bool IsMultiOptix => Type == DeviceType.DEVICE_MULTI && Subdevices.Any(device => device.IsOptix);
        public bool IsMultiMetal => Type == DeviceType.DEVICE_MULTI && Subdevices.Any(device => device.IsMetal);
        public bool IsMultiHip => Type == DeviceType.DEVICE_MULTI && Subdevices.Any(device => device.IsHip || device.IsHipRt);
        public bool IsMultiOneApi => Type == DeviceType.DEVICE_MULTI && Subdevices.Any(device => device.IsOneApi);

        /// <summary>
        /// Enumerate over sub-devices, returning a Tuple&lt;int, Device&gt; wher
        /// the int is index into global regular device list.
        /// </summary>
        public IEnumerable<Tuple<int, Device>> SubdevicesIndex
        {
            get
            {
                for (var i = 0; i < SubdeviceCount; i++)
                {
#if NOTYET
                    var j = CSycles.get_multidevice_subdevice_id((int)Id, i);
                    yield return new Tuple<int, Device>((int)j, GetDevice((int)j));
#else
                    yield return null;
#endif
                }
            }
        }

        public static string DeviceCapabilities(uint device_type_mask) {
            return CSycles.device_device_capabilities(device_type_mask);
        }

        public virtual bool IsReady(string value) {
            return CSycles.device_is_ready(Ptr, value);
        }

        public virtual void Cancel() {
            CSycles.device_cancel(Ptr);
        }

        public virtual bool ShouldUseGraphicsInterop() {
            return CSycles.device_should_use_graphics_interop(Ptr);
        }

        public virtual int DeviceNumber(IntPtr value) {
            return CSycles.device_device_number(Ptr, value);
        }

        public virtual string ErrorMessage() {
            return CSycles.device_error_message(Ptr);
        }

        public bool Headless {
            get { return CSycles.device_get_headless(Ptr); }
            set { CSycles.device_set_headless(Ptr, value); }
        }

        public virtual void OptimizeForScene(IntPtr value) {
            CSycles.device_optimize_for_scene(Ptr, value);
        }

        public virtual void SetError(string error) {
            CSycles.device_set_error(Ptr, error);
        }

        public bool HaveError() {
            return CSycles.device_have_error(Ptr);
        }

        public virtual IntPtr GetGuidingDevice() {
            return CSycles.device_get_guiding_device(Ptr);
        }
    }

}