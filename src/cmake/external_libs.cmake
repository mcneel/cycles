...
if(EXISTS ${_cycles_lib_dir})
  ...
  if(WIN32)
    if(EXISTS "${_cycles_lib_dir}/level-zero")
...
macro(add_bundled_libraries library_dir)
  if(IS_ABSOLUTE "${library_dir}")
    set(_library_dir "${library_dir}")
  elseif(DEFINED _cycles_lib_dir)
    set(_library_dir ${_cycles_lib_dir}/${library_dir})
  endif()

  if(DEFINED _library_dir)
    if(WIN32)
    ...
    if(WIN32)
      foreach(_bundled_lib ${_bundled_libs})
        if((${_bundled_lib} MATCHES "_d.dll$") OR
           (${_bundled_lib} MATCHES "_debug.dll$") OR
           (${_bundled_lib} MATCHES "sycl[0-9]*d.dll$"))
...
if(WITH_CYCLES_DEVICE_ONEAPI)
  ...
  if(SYCL_FOUND AND SYCL_VERSION VERSION_GREATER_EQUAL 6.0 AND LEVEL_ZERO_FOUND)
    ...
    if(WIN32)
      get_filename_component(_sycl_bin_dir "${SYCL_COMPILER}" DIRECTORY)
      if(EXISTS "${_sycl_bin_dir}")
        add_bundled_libraries("${_sycl_bin_dir}")
      endif()

      if(LEVEL_ZERO_LIBRARY MATCHES "\\.lib$")
        get_filename_component(_level_zero_lib_dir "${LEVEL_ZERO_LIBRARY}" DIRECTORY)
        get_filename_component(_level_zero_root_dir "${_level_zero_lib_dir}" DIRECTORY)
      else()
        set(_level_zero_root_dir "${LEVEL_ZERO_ROOT_DIR}")
      endif()

      if(EXISTS "${_level_zero_root_dir}/bin")
        add_bundled_libraries("${_level_zero_root_dir}/bin")
      endif()

      unset(_sycl_bin_dir)
      unset(_level_zero_lib_dir)
      unset(_level_zero_root_dir)
    endif()
