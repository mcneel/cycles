...
ccl::ShaderNode* _shader_node_find(ccl::Session* session_id, unsigned int shader, unsigned int shnode_id)
{
	assert(false);
#if LEGACY_SHADERS
	ccl::Scene* sce = nullptr;
	if (scene_find(session_id, &sce)) {
		CCShader* sh = csce->shaders[shader];
...
{
	int count = 0;
	for(ccl::ShaderNode *shn : shader->graph->nodes) {
		if(count == idx) {
			return shn;
		}
		count++;
...
CCL_CAPI void CDECL cycles_scene_tag_shader(ccl::Session *session_id, ccl::Shader *shader, bool use)
{
	ccl::Scene *sce = nullptr;
	if (scene_find(session_id, &sce)) {
		shader->tag_update(sce);
		if (use) {
			shader->tag_used(sce);
		}
	}
...
CCL_CAPI unsigned int CDECL cycles_scene_shader(ccl::Session *session_id, unsigned int shader)
{
	assert(false);
#if LEGACY_SHADERS
	ccl::Scene *sce = nullptr;
							   ...
							   unsigned int use_mis)
{
	...
	// if (shader)
	//	shader->set_volume_sampling_method(use_mis == 1 ?
...
{
	if (shader)
		shader->set_use_transparent_shadow(use_transparent_shadow == 1);
}
...
{
	if (shader)
		shader->set_heterogeneous_volume(heterogeneous_volume == 1);
}

CCL_CAPI ccl::ShaderNode* CDECL cycles_add_shader_node(ccl::Shader *shader,
										const char *node_type_name,
										const char *name)
{
	const ccl::NodeType *node_type = ccl::NodeType::find(ustring(node_type_name));
	ccl::ShaderNode *node = shader->graph->create_node(node_type);
												 ...
												 ccl::TextureMapping::Projection tm_projection)
{
	assert(false);
	/*
	ccl::ShaderNode* shnode = _shader_node_find(session_id, shader, shnode_id);
	if (shnode) {
		switch (shn_type) {
...
/*
Set an integer attribute with given name to value. shader is the global shader ID.
*/
...
/*
 * Set a float attribute with given name to value. shader is the global shader ID.
 */
...
/*
Set a vector of floats attribute with given name to x, y and z. shader is the global shader ID.
*/
...
CCL_CAPI bool CDECL cycles_shader_connect_nodes(ccl::Shader *shader,
...
CCL_CAPI void CDECL cycles_shader_disconnect_node(ccl::Shader *shader,
								   ccl::ShaderNode *from_node,
								   const char *from)
{
	assert(shader);
