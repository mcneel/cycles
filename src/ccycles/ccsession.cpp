...
static void prep_session(ccl::Session *session, std::vector<std::unique_ptr<CCyclesPassOutput>> *passes, CCSession* ccsession)
{
...

	{
		...
		shn = (ShaderNode *)graph_ptr->create_node(ntype);
		{
			std::random_device r;
...

	{
		...
		auto out = graph_ptr->output();
		ustring nodename("diffuse_bsdf");
		ccl::ShaderNode* shader_node = nullptr;
		const ccl::NodeType *ntype = ccl::NodeType::find(nodename);
		shader_node = (ShaderNode *)graph_ptr->create_node(ntype);
		{
			std::random_device r;
			std::mt19937 gen(r());	 // Standard mersenne_twister_engine seeded with rd()
			std::uniform_real_distribution<> dist(0.0, 1.0);
...
CCL_CAPI void CDECL cycles_session_destroy(ccl::Session* _session)
{
	CCSession* ccsess = nullptr;
	ccl::Session* session = nullptr;
	if (session_find(_session, &ccsess, &session)) {
...
CCL_CAPI void CDECL cycles_session_clear_passes(ccl::Session* _session)
{
	for (ccl::Pass *pass : _session->scene->passes) {
		_session->scene->delete_node(pass);
	}

	_session->scene->passes.clear();
...
CCL_CAPI void CDECL cycles_session_add_pass(ccl::Session *_session, int pass_id)
{
	ccl::PassType passtype = (ccl::PassType)pass_id;

	ccl::Pass *pass = _session->scene->create_node<ccl::Pass>();
...
CCL_CAPI int CDECL cycles_session_reset(ccl::Session* _session, int width, int height, int samples, int full_x, int full_y, int full_width, int full_height, int pixel_size)
{
	int rc = 0;
	CCSession* ccsess = nullptr;
	ccl::Session* session = nullptr;
	if (session_find(_session, &ccsess, &session)) {
		try {
			logger.logit("Reset session ", _session, ". width ", width, " height ", height, " samples ", samples);
...
CCL_CAPI void CDECL cycles_session_cancel(ccl::Session* _session, const char *cancel_message)
{
	CCSession* ccsess = nullptr;
	ccl::Session* session = nullptr;
	if (session_find(_session, &ccsess, &session)) {
		logger.logit("Cancel session ", _session, " with message ", cancel_message);
...
CCL_CAPI void CDECL cycles_session_start(ccl::Session* _session)
{
	CCSession* ccsess = nullptr;
	ccl::Session* session = nullptr;
	if (session_find(_session, &ccsess, &session)) {
		logger.logit("Starting session ", _session);
		session->start();
	}
}

CCL_CAPI void CDECL cycles_session_wait(ccl::Session* _session)
{
	CCSession* ccsess = nullptr;
	ccl::Session* session = nullptr;
	if (session_find(_session, &ccsess, &session)) {
		logger.logit("Waiting for session ", _session);
		session->wait();
	}
}

CCL_CAPI void CDECL cycles_session_set_pause(ccl::Session* _session, bool pause)
{
	CCSession* ccsess = nullptr;
	ccl::Session* session = nullptr;
	if (session_find(_session, &ccsess, &session)) {
...
CCL_CAPI void CDECL cycles_session_set_samples(ccl::Session* _session, int samples)
{
	CCSession* ccsess = nullptr;
	ccl::Session* session = nullptr;
	if (session_find(_session, &ccsess, &session)) {
...
CCL_CAPI void CDECL cycles_session_retain_float_buffer(
	ccl::Session *_session, int passtype, int width, int height, float **pixels, int* pixel_size)
{
	CCSession *ccsess = nullptr;
	ccl::Session *session = nullptr;
	if (session_find(_session, &ccsess, &session)) {
...
CCL_CAPI void CDECL cycles_session_release_float_buffer(ccl::Session *_session,
										 int passtype)
{
	CCSession *ccsess = nullptr;
	ccl::Session *session = nullptr;
...
CCL_CAPI void CDECL cycles_progress_reset(ccl::Session *_session)
{
	CCSession* ccsess = nullptr;
	ccl::Session* session = nullptr;
	if (session_find(_session, &ccsess, &session)) {
...
CCL_CAPI int CDECL cycles_progress_get_sample(ccl::Session* _session)
{
	CCSession* ccsess = nullptr;
	ccl::Session* session = nullptr;
	if (session_find(_session, &ccsess, &session)) {
...
CCL_CAPI int CDECL cycles_progress_get_rendered_tiles(ccl::Session *_session)
{
	CCSession *ccsess = nullptr;
	ccl::Session *session = nullptr;
	if (session_find(_session, &ccsess, &session)) {
...
CCL_CAPI void CDECL cycles_progress_get_time(ccl::Session* _session, double *total_time, double* sample_time)
{
	CCSession* ccsess = nullptr;
	ccl::Session* session = nullptr;
	if (session_find(_session, &ccsess, &session)) {
...
CCL_CAPI void CDECL cycles_progress_get_progress(ccl::Session* _session, float* progress)
{
	CCSession* ccsess = nullptr;
	ccl::Session* session = nullptr;
	if (session_find(_session, &ccsess, &session)) {
...
CCL_CAPI bool CDECL cycles_progress_get_status(ccl::Session* _session, void* strholder)
{
	CCSession* ccsess = nullptr;
	ccl::Session* session = nullptr;
	if (session_find(_session, &ccsess, &session)) {
...
CCL_CAPI bool CDECL cycles_progress_get_substatus(ccl::Session* _session, void* strholder)
{
	CCSession* ccsess = nullptr;
	ccl::Session* session = nullptr;
	if (session_find(_session, &ccsess, &session)) {
