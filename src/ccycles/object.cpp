...
unsigned int cycles_scene_add_clipping_plane(ccl::Session* session_id, float a, float b, float c, float d)
{
	ccl::Scene* sce = nullptr;
	if(scene_find(session_id, &sce)) {
		ccl::unique_ptr<ccl::float4> cp_ptr = ccl::make_unique<ccl::float4>();
		ccl::float4 *cp = cp_ptr.get();
		cp->x = a;
		cp->y = b;
		cp->z = c;
		cp->w = d;
		sce->clipping_planes.push_back(std::move(cp_ptr));
...
void cycles_scene_set_clipping_plane(ccl::Session* session_id, unsigned int cp_id, float a, float b, float c, float d)
{
	ccl::Scene* sce = nullptr;
	if(scene_find(session_id, &sce)) {
		ccl::float4* cp = sce->clipping_planes[cp_id];
		cp->x = a;
		cp->y = b;
		cp->z = c;
		cp->w = d;
