...
void ObjectManager::device_update_prim_offsets(Device *device, DeviceScene *dscene, Scene *scene)
{
  ...
  const uint object_count = scene->objects.size();
  ...
  for (Object *ob : scene->objects) {
    ...
    /* Rhino: guard against an out-of-range device index, which has caused
     * crashes here in RhinoCycles. */
    if (obj_index < object_count) {
