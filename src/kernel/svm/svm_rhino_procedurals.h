...
ccl_device_rhino_inline float4 impulse_noise(KernelGlobals kg, int x)
{
  ...
  noise.y = kernel_data_fetch(lookup_table, kernel_data.tables.rhino_impulse_noise_offset + x++);
  noise.z = kernel_data_fetch(lookup_table, kernel_data.tables.rhino_impulse_noise_offset + x++);
  noise.w = kernel_data_fetch(lookup_table, kernel_data.tables.rhino_impulse_noise_offset + x++);
                                ...
                                float contrast2)
{
  ...
  if (hillParameter > hillMiddle - 0.5f * hillLength) {
    if (hillParameter < hillMiddle + 0.5f * hillLength) {
      value = 0.5f + 0.5f * sinf((hillParameter - hillMiddle) / (hillLength)*PI);
    }
    else
      value = 1.0f;
  }
...
ccl_device float3 LogMapYxy(float3 Yxy, float exposure, float world_luminance, float max_luminance)
{
  ...
  float fExposure = powf(2.0f, exposure);  // default exposure is 1, 2^0
  ...
  if (fValue < 1.0f) {
    ...
    // Normal log(x+1).
    fLog = logf(fValue + 1.0f);
  }

  float fInterpol = logf(2.0f + powf(fValue / fLmax, fBiasP) * 8.0f);
...
ccl_device float3 hdr(float3 LDR, float exposure)
{
  ...
  return make_float3(1.0f, 1.0f, 1.0f) -
         make_float3(expf(fExposure * LDR.x), expf(fExposure * LDR.y), expf(fExposure * LDR.z));
}
                                       ...
                                       float exposure)
{
  ...
  float fMie = M1 * (1.0f + fCos * fCos) / powf(M2 - M3 * fCos, M4);
...
ccl_device float4 dots_tree_data_fetch(KernelGlobals kg, int x)
{
  ...
  float4 tree_data;
  tree_data.x = kernel_data_fetch(lookup_table,
                                 kernel_data.tables.rhino_dots_tree_data_offset + x);
  tree_data.y = kernel_data_fetch(lookup_table,
                                 kernel_data.tables.rhino_dots_tree_data_offset + x + 1);
  tree_data.z = kernel_data_fetch(lookup_table,
                                 kernel_data.tables.rhino_dots_tree_data_offset + x + 2);
  tree_data.w = kernel_data_fetch(lookup_table,
...
ccl_device float4 GetDotData(KernelGlobals kg, int index)
{
  ...
  float4 dot_data;
  dot_data.x = kernel_data_fetch(lookup_table,
                                kernel_data.tables.rhino_dots_dot_data_offset + index);
  dot_data.y = kernel_data_fetch(lookup_table,
                                kernel_data.tables.rhino_dots_dot_data_offset + index + 1);
  dot_data.z = kernel_data_fetch(lookup_table,
                                kernel_data.tables.rhino_dots_dot_data_offset + index + 2);
  dot_data.w = kernel_data_fetch(lookup_table,
