...
ccl_device float4 film_map(KernelGlobals *kg, float4 irradiance, float scale, int skip_linear_to_srgb_conversion)
{
	float exposure = kernel_data.film.exposure;
	float4 result = irradiance*scale;

	/* conversion to srgb unless skip conversion is set */
	result.x = skip_linear_to_srgb_conversion==1 ? result.x*exposure : color_scene_linear_to_srgb(result.x*exposure);
	result.y = skip_linear_to_srgb_conversion==1 ? result.y*exposure : color_scene_linear_to_srgb(result.y*exposure);
	result.z = skip_linear_to_srgb_conversion==1 ? result.z*exposure : color_scene_linear_to_srgb(result.z*exposure);
	...
	int skip_linear_to_srgb_conversion)
{
	...
	float4 float_result = film_map(kg, irradiance, sample_scale, skip_linear_to_srgb_conversion);
...
ccl_device void kernel_film_convert_to_half_float(KernelGlobals *kg,
	ccl_global uchar4 *rgba, ccl_global float *buffer,
	float sample_scale, int x, int y, int offset, int stride, int skip_linear_to_srgb_conversion)
{
	/* buffer offset */
	int index = offset + x + y*stride;
